...
Como construímos el modelo sobre la Casen 2017, introducimos 8 variables de ingreso, una de ruralidad, una de nivel educativo y otra de pobreza.


las convertimos a numérico.
...
prob_pred = predict(classifier, type = "response",
                    newdata = testing_set[,-11])

y_pred = ifelse(prob_pred > 0.455, 1, 0)
...
poblacion 2020 Chile. Cantidad de analfabetos:
...
Tenemos un clasificador construído para el 2017 pero vamos a calcular su presición para el 2015:
...
```{r, warning=FALSE}
data_2015w <- casen_2015[,c("e6a","zona","yaut", "ysub", "ytot", "ytrabajoCor","ytrabajoCorh", "yoprCorh","yoprCor","ymoneCorh","e1")]

names(data_2015w)[6] <- "ytrabajocor"
names(data_2015w)[7] <- "ytrabajocorh"
names(data_2015w)[8] <- "yoprcorh"
names(data_2015w)[9] <- "yoprcor"
names(data_2015w)[10] <- "ymonecorh"
...
datatable(head(data_2015w,10), escape = FALSE, rownames = FALSE,
          options = list( scrollX = TRUE))
...
```{r}
cm = table(data_2015w$e1, data_2015w$e1_pred)
cm <- as.data.frame(cm)
cm <- as.data.table(cm)
cm
```
